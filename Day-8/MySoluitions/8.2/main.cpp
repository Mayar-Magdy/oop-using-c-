...
int main()
{
    ...
    int shapeNum = 9;
    Shape* shapePtr[shapeNum];
// pointer and every ele in it point to pionter
    for(int i=0; i<3; ++i)
    {
        shapePtr[i] = &Rect_Arr[i];
        shapePtr[i+3] = &Cir_Arr[i];
        shapePtr[i+6] = &Line_Arr[i];
    }

    pic = Picture(shapeNum, shapePtr);
    pic.paint();
